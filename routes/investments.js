...
router.get("/result", async (req, res, next) => {
  try {
    ...
    const getReturnMultiplier = (rank) => {
      switch (rank) {
        case 1: return 3.0; // 1위: 3배
        case 2: return 2.0; // 2위: 2배
        case 3: return 1.0; // 3위: 원금
        case 4: return 0.8; // 4위: 0.8배
        case 5: return 0.5; // 5위: 0.5배
        default: return 0.0; // 그 외: 0배
      }
    };
...
router.get("/settlement/:userId", async (req, res, next) => {
  try {
    ...
    if (!investments || investments.length === 0) {
      return res.status(404).json({ error: "해당 유저의 investment 데이터가 없습니다." });
    }
